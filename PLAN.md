...
- [ ] zstd compression with optional dictionary support — Add zstd as CompressionId 4. Key design questions: (1) Dictionary storage: embed dictionary bytes in a new DICT attribute inside the LP envelope vs. reference an external file path — embedding is self-contained but bloats small archives, referencing is compact but requires the dict file at decompression time. (2) CLI UX: `-z zstd` for plain zstd, `-z zstd --dict <path>` to train/attach a dictionary. (3) LP attribute: new DICT sigil (e.g. 0x81 0x14) carrying either raw dict bytes or a reference identifier, sorted after ENC in the attribute order. (4) Dictionary source: pre-trained dictionaries (user supplies .dict file) vs. content-derived (zstd trains on archive entries before compressing — better ratios for small similar files but slower and non-deterministic). (5) Dependency: use `pmarreck/zstdz` (Zig wrapper) or fork `facebook/zstd` with a `build.zig` like the lz4 approach.
