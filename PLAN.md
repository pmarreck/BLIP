...
- [ ] zstd dictionary support — zstd compression is now implemented (CompressionId 4, via `pmarreck/zstdz`). Remaining: leverage zstd's separate dictionary feature for better compression of small similar files. Key design questions: (1) Dictionary storage: embed dictionary bytes in a new DICT attribute inside the LP envelope vs. reference an external file path. (2) CLI UX: `-z zstd --dict <path>` to train/attach a dictionary. (3) LP attribute: new DICT sigil carrying raw dict bytes or a reference identifier. (4) Dictionary source: pre-trained dictionaries vs. content-derived (zstd trains on archive entries before compressing).
