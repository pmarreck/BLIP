...
- [ ] Per-file compression mode (resilient archives) — Currently blar compresses the entire serialized body as one solid LP container. A single corrupted byte means total loss past that point. Staged plan:
  - **Stage 1: Per-file compression** — Compress each file entry independently within the archive. This alone gives corruption resilience: corrupted files can be skipped, the rest extracted with a warning. Per-entry hashes become meaningful for identifying exactly which files are damaged. This should become the default compression mode, with solid mode available via `--solid` for maximum ratio.
  - **Stage 2: File extension sorting (blar only)** — In blar (not mblar), sort file entries by file extension before archiving when compression is enabled. Groups similar files together which improves per-file compression ratios (compressor state carries over between adjacent similar files in streaming mode). Disable with `--no-sort`. Similar to 7-Zip's approach (extension-based, not magic-byte MIME detection).
  - **Stage 3: Per-MIME-type zstd dictionaries** — When using zstd with per-file compression: (1) group files by extension/MIME type, (2) train a zstd dictionary per group on the fly, (3) store dictionaries in a MAP container within the archive keyed by MIME type, (4) compress each file with its group's dictionary. This gives dictionary-level compression gains without solid-archive fragility. If a dictionary is corrupted, only its group's files are lost — not the whole archive. Design questions: dictionary storage as MAP in archive header vs. inline, minimum sample threshold for training (zstd recommends 100+ samples), CLI UX (`-z zstd-dict` vs automatic when file count is high enough).
