...
/// Legacy v1 ContainerType — kept for backward compatibility during migration.
/// Maps old enum values to their v1 byte positions.
/// NOTE: In v2, container type IDs are separate from attribute sigils.
/// The old `raw` type is replaced by `data`, and `lzma2` is now a CompressionId.
...
/// Legacy: Convert a ContainerType to its 2-byte sentinel.
/// Deprecated: Use attrSentinel() for v2 attribute sigils.
pub fn typeSentinel(ct_val: ContainerType) [2]u8 {
    return .{ SENTINEL_BYTE, @intFromEnum(ct_val) };
}

/// Legacy: Try to parse a ContainerType from the first 2 bytes of a buffer.
/// Deprecated: Use parseAttrSigil() for v2 attribute parsing.
...
// =============================================================================
// Legacy v1 Tests (backward compatibility — must still pass)
// =============================================================================
...
test "parseType round-trips all container types" {
    inline for (std.meta.fields(ContainerType)) |field| {
        const ct_val: ContainerType = @enumFromInt(field.value);
        const sentinel = typeSentinel(ct_val);
        try testing.expectEqual(ct_val, parseType(&sentinel).?);
    }
}
