...
**Fidelity note.** A naive mapping is lossy in the JSON→BLIP direction: a JSON string could mean UTF8 *or* a binary leaf; a number hides its BLIP width and endianness; `{…}` could be DICT *or* MAP. Faithful round-tripping therefore uses a lightweight **typed form** where ambiguity matters (e.g. `{"$data":"<printable-binary>"}`, `{"$map":{…}}`, `{"$u32":…}`), falling back to the natural mapping otherwise. The exact typed-JSON schema is finalized alongside the RPC expression layer (a call's arguments are exactly this authoring problem) and is intentionally left open here.
